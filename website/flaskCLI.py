...
def test():
    ...
    matarial_suppliers = pd.read_excel('../import_data/matarial_supplier.xlsx', dtype={'contact_number': str, 'cooperating': bool})
    matarial_suppliers.replace(np.nan, None, inplace=True)
    for record in matarial_suppliers.to_dict('index').values():
        MatarialSupplierC().create(**record)
